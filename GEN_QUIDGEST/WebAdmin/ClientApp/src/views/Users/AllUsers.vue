<template>
	...
	<qtable 
		...
		<template #TA3="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'TA3' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					TA3 : { active: true, Cod: "TA3", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.TA3.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.TA3.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.TA3.Description),
					name: "TA3",
					slot_name: "TA3",
					sort: false,
					visibility: vm.Modules.TA3.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
