...
FOR /F "tokens=*" %%i IN ('docker ps -q --filter "ancestor=tune_archive-webadmin:latest"') DO docker exec -i %%i bash < docker_db_maintenance.sh


::-------------------For macOS users----------------------------------
:: If you're using macOS, please run the following two commands:
::docker compose up -d --build
::docker exec -i $(docker ps -q --filter ancestor=tune_archive-webadmin:latest | head -n 1) bash < docker_db_maintenance.sh
